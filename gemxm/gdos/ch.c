...
GLOBAL BYTE	gl_vdrivechar = 0; /* primary drive selection mode:	 */
				   /* 0 = no primary drive		 */
...
GLOBAL LONG	ad_softint = 0L;	/* user software interrupt to be */
					...
					/* [JCE] Path to clipboard	 */
GLOBAL BYTE	gl_scrp[67] = "c:\\gemscrap\scrap.txt";
...
return;
		...
		case 30: /* make a snapshot file of a channel */
			...
			ch_snap(pid);
			break;
		case 81: /* [JCE] SCRP_WRITE */
			cmd = LLGET(ad_intin+0L);
			lstcpy(ADDR(gl_scrp), cmd);
			break;
	}
