...
/* [JCE] CTRL_CNT has to be 3 if you are going to treat addr_out 
 * sensibly. Otherwise its size is set by the caller and promptly
 * trashed by gem_if(). In single-tasking GEM this doesn't appear
 * to matter since only rsrc_gaddr() uses addr_out, and the next 
 * byte in ctrl_cnts is 2 so there is room in the array for the
 * return address. But in multi-tasking GEM there are other functions
 * with return addresses to consider. */
#define CTRL_CNT	3		/*mike's version is 3*/
