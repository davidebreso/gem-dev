...
WORD wr_image(LPTREE tree, WORD which)
	...
	if (type == G_IMAGE)
		{
		if ( (bitblk = (LPBIT)tree[which].ob_spec) == (LPBIT)-1) return 1;
		w = bitblk->bi_wb;
		h = bitblk->bi_hl;
	...
	if (type == G_ICON)
		{
		if ( (iconblk = (LPICON)tree[which].ob_spec) == (LPICON)-1) return 1;
...
WORD wr_tedinfo(LPTREE tree, WORD which)
	...
	type = tree[which].ob_type & 0xFF;
	if ( !(type == G_TEXT  || type == G_BOXTEXT || 
		   type == G_FTEXT || type == G_FBOXTEXT)) return 1;
	if ( (tedinfo = (LPTEDI)tree[which].ob_spec) == (LPTEDI)-1) return 1;
...
VOID wrsrt_inclfile(BYTE *ext, BYTE *ptns[])
	...
	FOREVER
		...
		if (rcs_fhhndl) break;
		if (!wcc_form_error(errno)) return;
		}
...
*/		rcs_frhndl = fopen(rcs_rfile, "wb");
		if (rcs_frhndl) break;
		if (!wcc_form_error(errno)) return (FALSE);
		}
