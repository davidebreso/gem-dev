/*	GEMWRECT.C	01/25/84 - 05/27/85	Lee Jay Lorenzen	*/
	...
	REG ORECT	*new, *r, *p;
{
	     ...
	     (new->o_y + new->o_h > r->o_y) )
	{
	  ...
	  have_piece[WLEFT] = ( new->o_x > r->o_x );
	  have_piece[WRIGHT] = ( new->o_x + new->o_w < r->o_x + r->o_w );
	...
	return((ORECT *)NULLPTR);
}


	VOID
mkrect(tree, wh, d1, d2)
	LPTREE		tree;			/* place holder for everyobj */
	WORD		wh;
	WORD		d1,d2;
{
	REG WINDOW	*pwin;
	...
	new = &gl_mkrect;
						/* 			*/
	r = (ORECT *)((p = (ORECT *)(&pwin->w_rlist))->o_link);
						/* redo rectangle list	*/
	while ( r )
	{
