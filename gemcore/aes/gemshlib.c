...
#else
	    dos_exec(ad_scmd, LHIWD((LONG)ad_envrn),
...
*/
        ...
        VOID
sh_rdef(LPBYTE lpcmd, LPBYTE lpdir)
{
	SHELL		*psh;
...
*/
        ...
        VOID
sh_wdef(LPBYTE lpcmd, LPBYTE lpdir)
{
	SHELL		*psh;
...
sh_ldapp()	/* MULTIAPP version */
{
	...
	if (shname_buf[0]) strcpy(shname_buf,       &psh->sh_desk[0]);
	else			   strcpy(rs_str(STDESKTP), &psh->sh_desk[0]);
...
sh_ldapp()
{
	...
	do
	{
	  ...
	  if (badtry)
	  {
	    ret = fm_show(badtry, (UWORD *)NULLPTR, 1);
	    if (badtry == ALNOFIT)
	      break;
	    badtry = 0;
	  }
	  ...
	  do
	  {
	    ...
	    if ( sh_find(ad_scmd) )
	    {
	      ...
	      if (psh->sh_fullstep == 0)
	      {
	        ...
	        dos_exec(ad_scmd, ad_envrn, ad_stail);
#else
	        dos_exec(ad_scmd, LHIWD((LONG)ad_envrn), ad_stail, 
			 ad_s1fcb, ad_s2fcb);
	      ...
	      else if (psh->sh_fullstep == 1)
	      {
	        ...
	        dos_exec(ad_scmd, ad_envrn, ad_stail);
#else
	        gsx_exec(ad_scmd, LHIWD((LONG)ad_envrn), ad_stail, 
			 ad_s1fcb, ad_s2fcb);
#endif
...
/*  02/11/86 LKW end	*/
	    ...
	    else
	    {
	      if ( (gl_shgem) &&
		   (psh->sh_isdef) )
	      {
		ret = fm_show(ALOKDESK, (UWORD *)NULLPTR, 1);
		if (ret == 1)
		  retry = TRUE;
		else
...
WORD xs_getshell(LPBYTE buf)
{
	if (shname_buf[0]) 
	{
		LSTCPY(buf, ADDR(shname_buf));
		return 1;
	}
	LSTCPY(buf, ADDR(rs_str(STDESKTP)));
	return 0;
}


WORD xs_setshell(LPBYTE buf)
{
	if (LSTRLEN(buf) > 15) return 1;
	LSTCPY(ADDR(shname_buf), buf);
