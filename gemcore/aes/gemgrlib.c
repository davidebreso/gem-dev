	...
	WORD		out, x, y, w, h;
{
	WORD		rets[6];
					...
					/*   MOBLK		*/
	dsptch();
	if ( MU_BUTTON & ev_multi(MU_KEYBD | MU_BUTTON | MU_M1, (MOBLK *)&out, 
		(MOBLK *)NULLPTR, 0x0L, 0x0001ff00L, 0x0L, &rets[0]) )
	  return(FALSE);
	return(TRUE);
} /* gr_stilldn */
	...
	WORD		dowdht;
{
	do
	{
/* [JCE] Busywait for a little, otherwise this goes too fast to see on
         modern PCs.
  */
	  LONG waste;

	  for (waste = 0; waste < gl_boxtick; ++waste) dos_getticks();
